				...
				<LeftContainer>
					...
					<Nav isNavOpened={isNavOpened}>
						{navList.map((item, index) => (
							<NavItem key={index} title={item.title} type={item.type} src={item.src} />
						))}
						<FontAwesomeIcon
				...
				<MemberNav>
					...
					{isLogged ? (
						<>
							...
							<NavItemAfterLogin bg="#231815" onClick={() => dataStore.logout()}>
								登出
							</NavItemAfterLogin>
						</>
					) : (
					...
					<MobileNav>
						{isLogged ? (
							<>
								...
								<MobileNavItem onClick={() => mobileGoto(`${paths.profile}?type=member`)}>
									<img src={userImg} />
									會員中心
								</MobileNavItem>
							</>
						...
						{navList.map((item, index) => (
							<MobileNavItem key={index} onClick={() => mobileGoto(paths[item.type])}>
								<img src={item.src} />
								{item.title}
							</MobileNavItem>
						))}
